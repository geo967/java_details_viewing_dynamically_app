...
import com.example.userdetailsviewingdynamically.Ui.DynamicViewActivity;
import com.example.userdetailsviewingdynamically.Ui.FirstActivity;
        ...
        call.enqueue(new Callback<List<MainModel>>() {
            ...
            public void onResponse(Call<List<MainModel>> call, Response<List<MainModel>> response) {
                ...
                for (int i = 0; i < data.size(); i++) {
                    DynamicViewActivity.list.add(new MainModel(data.get(i).getName(), data.get(i).getUsername(), data.get(i).getEmail(), data.get(i).getPhone()
